  ...
  const renderConvItem = (conv: Conversation) => {
    ...
    return (
      <div
        ...
        className={`
          group relative flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-all text-xs select-none
          ${isActive 
            ? isLight
              ? 'bg-emerald-50 text-emerald-950 font-bold border border-emerald-300/80 shadow-[0_1px_2px_rgba(0,0,0,0.02)]'
              : isHighContrast
              ? 'bg-white text-black font-bold border-2 border-white'
              : 'bg-emerald-950/60 text-emerald-100 font-bold border border-emerald-800' 
            : isLight
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          <MessageSquare className={`w-3.5 h-3.5 shrink-0 ${
            isActive 
              ...
              : isLight ? 'text-slate-400 group-hover:text-slate-600' : 'text-slate-500 group-hover:text-slate-400'
          }`} />
          {isEditing ? (
            <input
              type="text"
            ...
            <span className="truncate leading-normal">{conv.title || 'محادثة استفسار'}</span>
          )}
        </div>
        ...
        {!isEditing && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              ...
              className={`p-1 rounded transition-colors ${
                isLight ? 'hover:bg-rose-100 text-slate-400 hover:text-rose-700' : 'hover:bg-rose-950 text-slate-400 hover:text-rose-400'
              }`}
              title="حذف المحادثة"
            >
    ...
    <>
      ...
      <aside
        className={`
          ...
          w-72 flex flex-col h-full transition-all duration-200 ease-out border-l shrink-0
          ${isLight
      ...
      >
        {/* Top Header / New Chat */}
        <div className={`p-3.5 space-y-2.5 border-b shrink-0 ${isLight ? 'border-slate-200' : isHighContrast ? 'border-white' : 'border-slate-800'}`}>
          <div className="flex items-center justify-between lg:hidden pb-1">
            <div className="flex items-center gap-2">
              <TaxAuthorityLogo className="w-5 h-5 rounded-full" />
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 text-xs font-bold py-2.5 px-3 rounded-lg transition-all cursor-pointer active:scale-[0.98] shadow-xs ${
              isHighContrast
          ...
          <div className="relative">
            <input
              ...
              className={`w-full text-xs py-2 pr-8 pl-3 rounded-lg outline-none transition-all ${
                isLight
                  ? 'bg-white border border-slate-200 focus:border-emerald-700 text-slate-900 placeholder:text-slate-400 shadow-2xs'
                  : isHighContrast
                  ? 'bg-black border border-white text-white placeholder:text-zinc-400'
                  : 'bg-slate-900 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder:text-slate-500'
              }`}
            />
            ...
            {searchQuery && (
              <button
                ...
                className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 hover:text-slate-600 font-semibold"
              >
                مسح
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-3.5">
          {conversations.length === 0 ? (
            <div className="text-center py-12 px-4 space-y-2">
              <MessageSquare className={`w-7 h-7 mx-auto ${isLight ? 'text-slate-300' : 'text-slate-700'}`} />
              <p className={`text-xs font-bold ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>لا توجد استفسارات سابقة</p>
              <p className={`text-[11px] leading-relaxed ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>
                اطرح استفساراً جديداً لبدء المحادثة ومطابقة القواعد الضريبية المعتمدة.
              </p>
            </div>
          ) : filtered.length === 0 ? (
            ...
            <>
              ...
              {pinnedConvs.length > 0 && (
                <div className="space-y-0.5">
                  <div className={`px-3 py-1 text-[10px] font-bold flex items-center gap-1.5 ${isLight ? 'text-emerald-800' : 'text-emerald-400'}`}>
              ...
              {todayConvs.length > 0 && (
                <div className="space-y-0.5">
                  <div className={`px-3 py-1 text-[10px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>اليوم</div>
              ...
              {yesterdayConvs.length > 0 && (
                <div className="space-y-0.5">
                  <div className={`px-3 py-1 text-[10px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>أمس</div>
              ...
              {pastWeekConvs.length > 0 && (
                <div className="space-y-0.5">
                  <div className={`px-3 py-1 text-[10px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>الأسبوع الماضي</div>
              ...
              {olderConvs.length > 0 && (
                <div className="space-y-0.5">
                  <div className={`px-3 py-1 text-[10px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>سابقة</div>
        ...
        <div className={`p-3 border-t shrink-0 ${
          isLight ? 'bg-white/80 border-slate-200' : isHighContrast ? 'bg-black border-white' : 'border-slate-800/80 bg-slate-900/40'
        }`}>
